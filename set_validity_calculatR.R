...
library(irr)
library(arm)
#library(peRspective) # don't need to really read it in right now
library(grid)
library(stargazer)
library(ROCR)
...
ml_models_set_df <- read.csv("final_data/openai_predictions2.csv")
...
#### Just redo the stuff for ONU here; we have teh data, after all 

onu_df <- subset(ml_models_set_df, college=="OHIO NORTHERN UNIVERSITY")

ggplot_persp_corr_onu <- ggplot(onu_df, aes(x=total_toxicity,y=(TOXICITY2)*100,
  ...
  labs(title="Comparison of correlation of manual coding on peRspective scores",
       ...
       caption=paste0("X-axis reflects the average of the two coders, with an ICC of 0.827. 
                      R-squared = 0.325, coef = 6.5"))+
  ...
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth")
ggplot_persp_corr_onu # ok, this is what we want; get ri of the \n, 
# grayscale, add transparency, change to triangles and squares
# 
ggsave("plots/coding_perspective_corr_plot_onu_gs.png" ,ggplot_persp_corr_onu, 
...
ggplot_persp_corr_all <- ggplot(ml_models_set_df, aes(x=total_toxicity,y=(TOXICITY2)*100,
  ...
  labs(title="Comparison of correlation of manual coding \n on peRspective scores",
       x="Aggregated manual coding of comments", y="peRspective Toxicity score", 
       caption=paste0("X-axis reflects the average of the two coders.  
                      R-squared = 0.3049, coef = 11.04, p<0.01, N=5,167"))+
  scale_color_manual(name = "Manual coding",
                       labels = c("Not toxic", "Toxic"),
                       values=c("gray70","gray20")) +
  scale_fill_manual(name = "Manual coding",
                     labels = c("Not toxic", "Toxic"),
                     values=c("gray70","gray20")) +
  scale_shape_manual(name="Manual coding",
                     labels = c("Not toxic", "Toxic"),
                     values=c(15,17)) +
  ylim(0,100)+
  stat_smooth(method = "lm",
              ...
              geom = "smooth",
              show.legend = F) +
  guides(color = guide_legend("Manual coding"), 
         size= guide_legend("Manual coding"),
         fill= guide_legend("Manual coding"))
...
ggsave("plots/complete_perspective_corr_plot_gs.png" ,ggplot_persp_corr_all, 
       scale=1,width=9,height=6,units = c("in"), dpi=400,bg="white")
ggsave("plots/complete_openai_corr_plot_gs.png" ,ggplot_openai_corr_all, 
       scale=1,width=9,height=6,units = c("in"), dpi=400,bg="white")
